...
// Helper to get the route from the URL
function getRoute(url: URL): string {
  const path = url.pathname;
  // Remove /functions/v1/institutions from the path
  let route = path.replace("/functions/v1/institutions", "");
...
Deno.serve(async (req) => {
  ...
  console.log(`[DEBUG] Method: ${method}, Route: "${route}"`);

  try {
    // ============================================================
    // ROUTE: "" - INSTITUTIONS (root)
    // ============================================================
    if (route === "") {
      if (method === "GET") {
    ...
    // ============================================================
    // ROUTE: "institutions/:id" - Single institution
    // ============================================================
    const idMatch = route.match(/^institutions\/(\d+)$/);
    if (idMatch) {
    ...
    // ============================================================
    // ROUTE: "bursaries/:id" - Single bursary
    // ============================================================
    const bursIdMatch = route.match(/^bursaries\/(\d+)$/);
    if (bursIdMatch) {
      const id = parseInt(bursIdMatch[1]);
    ...
    return _json({ 
      ...
      available_routes: [
        ...
        "GET /institutions/bursaries - List all bursaries",
        "POST /institutions/bursaries - Create bursary",
        ...
        "GET /institutions/bursaries/{id} - Get single bursary",
        "PUT /institutions/bursaries/{id} - Update bursary",
        "DELETE /institutions/bursaries/{id} - Delete bursary"
      ]
    }, 404);
